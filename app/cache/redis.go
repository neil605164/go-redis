...
func RedisConn() *redis.Client {
	...
	// 	IdleCheckFrequency: 10 * time.Second, //闲置连接检查的周期，默认为1分钟，-1表示不做周期性检查，只在客户端获取连接时对闲置连接进行处理。
	// 	IdleTimeout:        10 * time.Second, //闲置超时，默认5分钟，-1表示取消闲置超时检查
	// 	MaxConnAge:         10 * time.Second, //连接存活时长，从创建开始计时，超过指定时长则关闭连接，默认为0，即不关闭存活时长较长的连接
	...
	// 		}
	// 		return netDialer.Dial("tcp", "redis_go-redis:6379")
	// 	},
	...
	gClient = redis.NewClient(&redis.Options{
		Addr:     "redis_go-redis:6379",
		Password: "rd3Redis", // no password set
		DB:       0,          // use default DB
	})
